...
import FlowAssets from "./FlowAssets.cdc"
import NonFungibleToken from "./NonFungibleToken.cdc"

/*
    This is a simple FlowAssets initial sale contract for the DApp to use
    in order to list and sell FlowAssets.
...
pub contract FlowAssetsMarket {
    ...
    // SaleOffer
    // A FlowAssets NFT being offered to sale for a set fee paid in FlowToken.
    //
    pub resource SaleOffer: SaleOfferPublicView {
        ...
        // The FlowAssets NFT ID for sale.
        pub let itemID: UInt64
        ...
        access(self) let sellerItemProvider: Capability<&FlowAssets.Collection{NonFungibleToken.Provider}>

        // The Flow vault that will receive that payment if the sale completes successfully.
        access(self) let sellerPaymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>

        // The Flow vault that will receive the cut percentage payment if the sale completes successfully.
        access(self) let marketPaymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>

        // Called by a purchaser to accept the sale offer.
        // If they send the correct payment in Flow, and if the item is still available,
        // the FlowAssets NFT will be placed in their FlowAssets.Collection .
        //
        pub fun accept(
            buyerCollection: &FlowAssets.Collection{NonFungibleToken.Receiver},
        ...
        // to transfer the FlowAssets NFT and the capability to receive Flow in payment.
        //
        init(
            sellerItemProvider: Capability<&FlowAssets.Collection{NonFungibleToken.Provider}>,
            itemID: UInt64,
            templateID: UInt64,
            sellerPaymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>,
            marketPaymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>,
            price: UFix64,
            marketFee: UFix64,
        ) {
            pre {
    ...
    pub resource interface CollectionManager {
        pub fun insert(offer: @FlowAssetsMarket.SaleOffer)
    ...
    pub resource Collection : CollectionManager, CollectionPurchaser, CollectionPublic {
        ...
        // containing the correct payment amount, this will transfer the FlowAsset to the caller's
        ...
        //   2. FlowAssets.Withdraw
        //   3. FlowAssets.Deposit
    ...
    init () {
        ...
        self.CollectionStoragePath = /storage/flowAssetsMarketCollection002
        self.CollectionPublicPath = /public/flowAssetsMarketCollection002
    }
}
