...
//
pub contract FlowAssets: NonFungibleToken {
    ...
    // totalSupply
    // The total number of FlowAssets that have been minted
    //
    ...
    // -----------------------------------------------------------------------
    // FlowAssets contract-level Composite Type definitions
    // -----------------------------------------------------------------------
    ...
    pub struct Template {
        ...
        init(metadata: {String: String}) {
            ...
            self.templateID = FlowAssets.nextTemplateID
            self.metadata = metadata

            // Increment the ID so that it isn't used again
            FlowAssets.nextTemplateID = FlowAssets.nextTemplateID + 1 as UInt64
    ...
    pub resource Set {
        ...
        pub fun addTemplate(templateID: UInt64) {
            pre {
                FlowAssets.templateDatas[templateID] != nil: "Cannot add the Template to Set: Template doesn't exist."
    ...
    pub resource NFT: NonFungibleToken.INFT {
        ...
        init(mintNumber: UInt64, templateID: UInt64, setID: UInt64) {
            ...
            FlowAssets.totalSupply = FlowAssets.totalSupply + 1 as UInt64

            self.id = FlowAssets.totalSupply
    ...
    pub resource Admin {
        ...
        pub fun createTemplate(metadata: {String: String}): UInt64 {
            ...
            FlowAssets.templateDatas[newID] = newTemplate
        ...
        pub fun borrowSet(setID: UInt64): &Set {
            pre {
                FlowAssets.sets[setID] != nil: "Cannot borrow Set: The Set doesn't exist"
            }
            
            // Get a reference to the Set and return it
            // use `&` to indicate the reference to the object and type
            return &FlowAssets.sets[setID] as &Set
        }
    ...
    // This is the interface that users can cast their FlowAssets Collection as
    // to allow others to deposit FlowAssets into their Collection. It also allows for reading
    // the details of FlowAssets in the Collection.
    pub resource interface FlowAssetsCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun batchDeposit(tokens: @NonFungibleToken.Collection)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
        pub fun borrowFlowAsset(id: UInt64): &FlowAssets.NFT? {
    ...
    //
    pub resource Collection: FlowAssetsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        //
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @FlowAssets.NFT
    ...
    pub fun getAllTemplates(): [FlowAssets.Template] {
        return FlowAssets.templateDatas.values
    }
    ...
    pub fun getTemplateMetaDataByField(templateID: UInt64, field: String): String? {
        ...
        if let template = FlowAssets.templateDatas[templateID] {
    ...
    pub fun getSetName(setID: UInt64): String? {
        ...
        return FlowAssets.setDatas[setID]?.name
    }

    pub fun getSetMarketFee(setID: UInt64): UFix64? {
        // Don't force a revert if the setID is invalid
        return FlowAssets.setDatas[setID]?.marketFee
    }
    ...
    pub fun getSetIDsByName(setName: String): [UInt64]? {
        ...
        for setData in FlowAssets.setDatas.values {
    ...
    pub fun getTemplatesInSet(setID: UInt64): [UInt64]? {
        // Don't force a revert if the setID is invalid
        return FlowAssets.sets[setID]?.templates
    }
    ...
    pub fun isSetTemplateLocked(setID: UInt64, templateID: UInt64): Bool? {
        ...
        if let setToRead <- FlowAssets.sets.remove(key: setID) {
            ...
            FlowAssets.sets[setID] <-! setToRead
    ...
    pub fun isSetLocked(setID: UInt64): Bool? {
        // Don't force a revert if the setID is invalid
        return FlowAssets.sets[setID]?.locked
    }
    ...
    // If an account does not have a FlowAssets.Collection, panic.
    // If it has a collection but does not contain the itemID, return nil.
    // If it has a collection and that collection contains the itemID, return a reference to that.
    //
    pub fun fetch(_ from: Address, itemID: UInt64): &FlowAssets.NFT? {
        let collection = getAccount(from)
            .getCapability(FlowAssets.CollectionPublicPath)!
            .borrow<&FlowAssets.Collection{FlowAssets.FlowAssetsCollectionPublic}>()
            ?? panic("Couldn't get collection")
        // We trust FlowAssets.Collection.borowFlowAsset to get the correct itemID
    ...
    init() {
        ...
        self.account.link<&{FlowAssetsCollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)

        // Put the Minter in storage
        self.account.save<@Admin>(<- create Admin(), to: /storage/FlowAssetsAdmin)
    }
}
